...
internal static class VoiceGenerator
{
    ...
    public static void Explore()
    {
        ...
        var parsedVoices = JsonSerializer.Deserialize<IReadOnlyCollection<TextToSpeechVoiceDefinitionPso>>(VoicesJsonString)!;
        var voices = parsedVoices
            .Select(_ => _.ToModel())
            .Where(_ => _.Locale.ToLower().StartsWith("en"))
            .OrderBy(_ => (_.Gender, _.Locale, _.DisplayName))
            .ToList();

        var strOf = (string str) => $"\"{str}\"";

        foreach (var vo in voices)
        {
            var voiceVariableName = $"{vo.Locale.Replace("en", "En").Replace("-", "")}{vo.DisplayName}";
            var args = new List<(string Name, string Value)>
            {
                ("Name",strOf(vo.Name)),
                ("DisplayName", strOf(vo.DisplayName)),
                ("LocalName", strOf(vo.LocalName)),
                ("ShortName", strOf(vo.ShortName)),
                ("Gender", $"TextToSpeechGenderType.{vo.Gender.ToString()}"),
                ("Locale", strOf(vo.Locale)),
                ("LocaleName", strOf(vo.LocaleName)),
                ("SampleRateHertz", vo.SampleRateHertz + ""),
                ("VoiceType", $"TextToSpeechVoiceType.{vo.VoiceType.ToString()}"),
                ("Status", $"TextToSpeechVoiceStatus.{vo.Status.ToString()}"),
                ("WordsPerMinute", vo.WordsPerMinute + "")
            };
            Console.WriteLine($"public static readonly TextToSpeechVoiceDefinition {voiceVariableName} = new TextToSpeechVoiceDefinition(\r\n" +
                args
    ...
    /*
     * Generated by: GET: https://northeurope.tts.speech.microsoft.com/cognitiveservices/voices/list
     * Providing SpeechKey as Header: Ocp-Apim-Subscription-Key
     */
    private static string VoicesJsonString = @"
[
    {
